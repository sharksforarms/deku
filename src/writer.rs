...
impl<W: Write + Seek> Writer<W> {
    ...
    pub fn write_bits_order(
        ...
        let rest = if leftover.0.is_empty() {
            ...
            // Maintain order associated with buffer
            let (first, complement, rest) = if leftover.1 == Order::Msb0 {
                if order == Order::Msb0 {
                    ...
                    assert!(leftover.0.capacity() >= bits.len());
                    let complement = leftover.0.capacity() - bits.len();
                    let complement = core::cmp::min(complement, leftover.0.len());
                    let (complement, rest) = leftover.0.as_bitslice().split_at(complement);
                    ((complement, leftover.1), (bits, order), (rest, leftover.1))
                }
            } else if order == Order::Msb0 {
                assert!(leftover.0.capacity() >= bits.len());
                let complement = leftover.0.capacity() - bits.len();
                let complement = core::cmp::min(complement, leftover.0.len());
                let (complement, rest) = leftover.0.as_bitslice().split_at(complement);
            ...
            if self.leftover.0.is_full() {
                self.inner
                    .write_all(self.leftover.0.as_raw_slice())
        ...
        let remainder = if rest.1 == Order::Msb0 {
            let iter = rest.0.chunks_exact(bits_of::<u8>());
            let remainder = iter.remainder();
            for byte in iter {
                self.inner.write_all(&[byte.load_be()]).expect("Write fail");
            }
            remainder
        } else {
