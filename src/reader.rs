...
impl<R: Read + Seek> Reader<R> {
    ...
    pub fn skip_bits(&mut self, amt: usize, _order: Order) -> Result<(), DekuError> {
        #[cfg(feature = "bits")]
        {
            ...
            if let Some(Leftover::Bits(bits)) = &self.leftover {
                ...
                bits_amt -= needed;
                self.read_bits_into(_order, &mut buf[..needed])?;
            }
            ...
            let mut buf = bitarr!(u8, Msb0; 0; 8);
            self.read_bits_into(_order, &mut buf[..bits_amt])?;
        }
    ...
    pub fn read_bits(
        ...
        self.read_bits_into(order, vec.as_mut_bitslice())?;
        Ok(Some(vec))
    }
    ...
    fn read_bytes_other(
        ...
        match self.leftover {
            ...
            Some(Leftover::Bits(_)) => {
                ...
                self.read_bits_into(_order, slice)?;
                Ok(ReaderRet::Bytes)
            }
            _ => unreachable!(),
        }
